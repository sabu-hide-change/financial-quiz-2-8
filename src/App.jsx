...
const quizQuestions = [
  ...
  },
  {
    ...
    title: "リスクに対する投資家の選好",
    tag: "無差別曲線",
    text: "投資家の無差別曲線から判断して、投資家の選好を表す組み合わせとして最も適切なものを選べ。縦軸にリターン（期待収益率）、横軸にリスク（標準偏差）をとり、満足度のレベルはU1＜U2＜U3である。\n・曲線A：右上がりの凸曲線\n・直線B：水平な直線\n・曲線C：右下がりの凹曲線",
    options: [
    ...
    explanation: {
      ...
      details: [
        ...
        { label: "Ｃ：リスク愛好者", content: "同一のリターンならばリスクがより大きいものを選好する投資家。リスクを取ること自体を好むため、曲線は右下がりになります。" }
      ]
    }
  },
  {
  ...
  },
  {
    ...
    explanation: {
      ...
      details: [
        { label: "相関係数 ＝ －1", content: "2つの証券が完全に逆の動きをします。このときリスク低減効果は最大となり、特定の比率で全体のリスク（標準偏差）をゼロにすることが可能です。" },
        { label: "相関係数 ＝ 0", content: "2つの証券の動きには何の関係もありませんが、ポートフォリオを組むことでリスクを低減することができます。" },
        { label: "相関係数 ＝ 1", content: "2つの証券が完全に同じ動きをします。この場合はリスクが直線的に変化するだけで、分散投資によるリスク低減効果は一切ありません。" },
        { label: "結論", content: "したがって、相関係数が1以外であれば、大なり小なり必ずリスク低減効果が発揮されます。" }
      ]
    }
  },
  {
    ...
    explanation: {
      ...
      details: [
        { label: "Ａ：アンシステマティック・リスク", content: "銘柄固有の要因によるリスク。多数の多様な銘柄を組み合わせる（分散投資）ことで消去可能です（非システマティック・リスク）。" },
        { label: "Ｂ：システマティック・リスク", content: "景気後退や金利変動など、市場全体が受けるシステム起因のリスク。どんなに銘柄数を増やしても消去することはできません。" },
        { label: "Ｃ：証券市場線（SML）", content: "横軸に『β（ベータ値：市場感応度）』、縦軸に『期待リターン』をとる、個別資産の評価直線です（資本市場線は横軸に標準偏差をとるため混同注意）。" }
      ]
    }
  },
  {
    ...
    explanation: {
      ...
      details: [
        ...
        { label: "ウ・エ ○", content: "フロンティア線上の中であれば、投資家のリスクに対する好みに合わせて、左側（ローリスク）か右側（ハイリスク）のいずれかを選択することになります。" }
      ]
    }
  },
  {
    ...
    explanation: {
      ...
      details: [
        { label: "ア ×", content: "国債100%のときの期待収益率は縦軸の切片である2%です。" },
        { label: "イ ○", content: "国債100%のとき、リスクの指標である標準偏差は0となっています。これはリスクが一切ないことを意味し、リスクフリー資産の定義に完全に合致します。" },
  ...
  },
  {
    ...
    explanation: {
      ...
      details: [
        { label: "ア ×", content: "資本市場線は、安全資産とリスク資産（株式ポートフォリオ）を組み合わせた最適な直線のことです。" },
  ...
  },
  {
    ...
    explanation: {
      ...
      details: [
        ...
        { label: "エ ×", content: "間接金融は、銀行などの金融仲介機関が預金者から集めた資金を、機関の判断とリスクのもとで企業へ融資する方法です。仲介機関を経由するのが特徴です。" }
      ]
    }
  },
  {
